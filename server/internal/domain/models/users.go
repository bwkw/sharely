...
type User struct {
	...
	Sex       int         `boil:"sex" json:"sex" toml:"sex" yaml:"sex"`
...
var UserWhere = struct {
	...
	Sex:       whereHelperint{field: "`users`.`sex`"},
...
var (
	userAllColumns            = []string{"id", "sex", "name", "email", "password", "image", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"sex", "name", "email", "password", "image", "created_at", "updated_at"}
...
// CreatorGroups retrieves all the group's Groups with an executor via creator_id column.
func (o *User) CreatorGroups(mods ...qm.QueryMod) groupQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`groups`.`creator_id`=?", o.ID),
	)
...
// SenderInvitations retrieves all the invitation's Invitations with an executor via sender_id column.
func (o *User) SenderInvitations(mods ...qm.QueryMod) invitationQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`invitations`.`sender_id`=?", o.ID),
	)
...
// ReceiverInvitations retrieves all the invitation's Invitations with an executor via receiver_id column.
func (o *User) ReceiverInvitations(mods ...qm.QueryMod) invitationQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`invitations`.`receiver_id`=?", o.ID),
	)
...
// LoadCreatorGroups allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatorGroups(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`groups`),
		qm.WhereIn(`groups.creator_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CreatorGroups = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CreatorID {
				local.R.CreatorGroups = append(local.R.CreatorGroups, foreign)
				if foreign.R == nil {
					foreign.R = &groupR{}
				}
...
// LoadSenderInvitations allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadSenderInvitations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`invitations`),
		qm.WhereIn(`invitations.sender_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.SenderInvitations = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.SenderID {
				local.R.SenderInvitations = append(local.R.SenderInvitations, foreign)
				if foreign.R == nil {
					foreign.R = &invitationR{}
				}
...
// LoadReceiverInvitations allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadReceiverInvitations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`invitations`),
		qm.WhereIn(`invitations.receiver_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ReceiverInvitations = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &invitationR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ReceiverID {
				local.R.ReceiverInvitations = append(local.R.ReceiverInvitations, foreign)
				if foreign.R == nil {
					foreign.R = &invitationR{}
				}
...
// AddCreatorGroups adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatorGroups.
// Sets related.R.Creator appropriately.
func (o *User) AddCreatorGroups(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Group) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			CreatorGroups: related,
		}
	} else {
		o.R.CreatorGroups = append(o.R.CreatorGroups, related...)
	}
...
// AddSenderInvitations adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.SenderInvitations.
// Sets related.R.Sender appropriately.
func (o *User) AddSenderInvitations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Invitation) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.SenderID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			SenderInvitations: related,
		}
	} else {
		o.R.SenderInvitations = append(o.R.SenderInvitations, related...)
	}
...
// AddReceiverInvitations adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.ReceiverInvitations.
// Sets related.R.Receiver appropriately.
func (o *User) AddReceiverInvitations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Invitation) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ReceiverID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			ReceiverInvitations: related,
		}
	} else {
		o.R.ReceiverInvitations = append(o.R.ReceiverInvitations, related...)
	}
