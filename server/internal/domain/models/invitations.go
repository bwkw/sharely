...
type Invitation struct {
	...
	ReceiverID int       `boil:"receiver_id" json:"receiver_id" toml:"receiver_id" yaml:"receiver_id"`
...
var InvitationTableColumns = struct {
	...
	ID:         "invitations.id",
	SenderID:   "invitations.sender_id",
	ReceiverID: "invitations.receiver_id",
...
var InvitationWhere = struct {
	...
	ReceiverID whereHelperint
	GroupID    whereHelperint
	CreatedAt  whereHelpernull_Time
	UpdatedAt  whereHelpernull_Time
}{
	ID:         whereHelperint{field: "`invitations`.`id`"},
	SenderID:   whereHelperint{field: "`invitations`.`sender_id`"},
	ReceiverID: whereHelperint{field: "`invitations`.`receiver_id`"},
	GroupID:    whereHelperint{field: "`invitations`.`group_id`"},
...
func (o *Invitation) Sender(mods ...qm.QueryMod) userQuery {
...
func (o *Invitation) Receiver(mods ...qm.QueryMod) userQuery {
	queryMods := []qm.QueryMod{
...
func (o *Invitation) Group(mods ...qm.QueryMod) groupQuery {
	queryMods := []qm.QueryMod{
...
func (invitationL) LoadSender(ctx context.Context, e boil.ContextExecutor, singular bool, maybeInvitation interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.SenderInvitations = append(foreign.R.SenderInvitations, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.SenderID == foreign.ID {
				...
				foreign.R.SenderInvitations = append(foreign.R.SenderInvitations, local)
				break
			}
		}
	}
...
// loaded structs of the objects. This is for an N-1 relationship.
func (invitationL) LoadReceiver(ctx context.Context, e boil.ContextExecutor, singular bool, maybeInvitation interface{}, mods queries.Applicator) error {
	var slice []*Invitation
	var object *Invitation
	...
	if singular {
		...
		foreign.R.ReceiverInvitations = append(foreign.R.ReceiverInvitations, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.ReceiverID == foreign.ID {
				...
				foreign.R.ReceiverInvitations = append(foreign.R.ReceiverInvitations, local)
				break
			}
		}
	}
...
// loaded structs of the objects. This is for an N-1 relationship.
func (invitationL) LoadGroup(ctx context.Context, e boil.ContextExecutor, singular bool, maybeInvitation interface{}, mods queries.Applicator) error {
	var slice []*Invitation
	var object *Invitation
	...
	if singular {
		...
		foreign.R.Invitations = append(foreign.R.Invitations, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.GroupID == foreign.ID {
				...
				foreign.R.Invitations = append(foreign.R.Invitations, local)
				break
			}
		}
	}
...
// SetSender of the invitation to the related item.
// Sets o.R.Sender to related.
// Adds o to related.R.SenderInvitations.
func (o *Invitation) SetSender(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &userR{
			SenderInvitations: InvitationSlice{o},
		}
	} else {
		related.R.SenderInvitations = append(related.R.SenderInvitations, o)
	}

	return nil
}

// SetReceiver of the invitation to the related item.
// Sets o.R.Receiver to related.
// Adds o to related.R.ReceiverInvitations.
func (o *Invitation) SetReceiver(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &userR{
			ReceiverInvitations: InvitationSlice{o},
		}
	} else {
		related.R.ReceiverInvitations = append(related.R.ReceiverInvitations, o)
	}

	return nil
}

// SetGroup of the invitation to the related item.
// Sets o.R.Group to related.
// Adds o to related.R.Invitations.
func (o *Invitation) SetGroup(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Group) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &groupR{
			Invitations: InvitationSlice{o},
		}
	} else {
		related.R.Invitations = append(related.R.Invitations, o)
	}
...
func FindInvitation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Invitation, error) {
...
func (o *Invitation) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == invitationMapping["id"] {
		goto CacheNoHooks
	}
...
func InvitationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
